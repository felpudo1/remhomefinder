...
export function AdminPrompt({ toast }: Props) {
  ...
  const handleSave = async (type: "user" | "agent") => {
    ...
    toast({ title: "Prompt guardado", description: `El prompt de ${type === "user" ? "usuarios" : "agentes"} se actualizó correctamente.` });
  };

  const handleReset = (type: "user" | "agent") => {
    if (type === "user") {
      setPromptUser(DEFAULT_PROMPT_USER);
      setSavedUser(false);
    } else {
      setPromptAgent(DEFAULT_PROMPT_AGENT);
      setSavedAgent(false);
    }
    ...
    <div className="space-y-8">
      ...
      <div className="flex items-start gap-3 p-4 bg-primary/5 border border-primary/20 rounded-xl text-sm text-muted-foreground">
        ...
        <div>
          ...
          <p>Son las instrucciones que le damos a la IA para interpretar avisos inmobiliarios. El prompt de <strong>usuarios</strong> se usa cuando un usuario agrega una propiedad. El de <strong>agentes</strong> cuando una agencia publica desde su panel.</p>
        </div>
      </div>
