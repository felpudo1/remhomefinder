...
export function generateSentimentInsight(
  ...
  switch (status) {
    case "descartado": {
      const matchPercent = allRatings.length > 0 ? Math.max(0, 30 - ratingToPct(avg(allRatings)) / 4) : 15;
      const summary = motivo
        ? `Descartado: ${motivo.slice(0, 90)}`
        : "Lead descartado sin motivo claro. Revisar feedback del usuario.";
      return { summary: truncateWords(summary, 15), matchPercent: Math.round(matchPercent) };
    }

    case "firme_candidato": {
      const promedio = avg(allRatings);
      const matchPercent = promedio > 0 ? Math.min(100, ratingToPct(promedio) + 10) : 85;
      const summary = motivo
    ...
    case "posible_interes": {
      const promedio = avg(allRatings);
      const matchPercent = promedio > 0 ? Math.round(ratingToPct(promedio) * 0.7) : 45;
      const summary = motivo
        ? `Tibio: ${motivo.slice(0, 85)}`
        : "Lead tibio, falta info o precio para escalar el interés.";
      return { summary: truncateWords(summary, 15), matchPercent };
    }

    case "contactado": {
      const promedio = avg(allRatings);
      const matchPercent = promedio > 0 ? ratingToPct(promedio) : 50;
      const summary = motivo
    ...
    case "visita_coordinada": {
      const promedio = avg(allRatings);
      const matchPercent = promedio > 0 ? Math.min(100, ratingToPct(promedio) + 5) : 70;
      const summary = motivo
