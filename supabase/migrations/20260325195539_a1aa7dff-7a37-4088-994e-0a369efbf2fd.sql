...
-- ============================================================
-- RPC: get_user_listings_page
-- Replaces ~10 client-side queries with a single round-trip.
-- Uses SECURITY INVOKER so RLS on base tables applies via auth.uid().
-- ============================================================

-- Helper table type not needed; we return jsonb directly.

CREATE OR REPLACE FUNCTION public.get_user_listings_page(
...
REVOKE ALL ON FUNCTION public.get_user_listings_page(timestamptz, int) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.get_user_listings_page(timestamptz, int) TO authenticated;
