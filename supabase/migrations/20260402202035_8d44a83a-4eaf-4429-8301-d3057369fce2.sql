...
BEGIN
    ...
    -- Leer referral_id del metadata (pasado durante signUp)
    BEGIN
        v_referral_id := (NEW.raw_user_meta_data->>'referral_id')::uuid;
    EXCEPTION WHEN OTHERS THEN
        v_referral_id := NULL;
    END;

    -- Prevenir auto-referencia
    IF v_referral_id = NEW.id THEN
        v_referral_id := NULL;
    END IF;

    INSERT INTO public.profiles (user_id, display_name, phone, email, avatar_url, status, referred_by_id)
    VALUES (
        NEW.id,
        v_display_name,
        v_phone,
    ...
    ON CONFLICT (user_id) DO UPDATE SET
        ...
        referred_by_id = CASE WHEN profiles.referred_by_id IS NULL THEN EXCLUDED.referred_by_id ELSE profiles.referred_by_id END;
